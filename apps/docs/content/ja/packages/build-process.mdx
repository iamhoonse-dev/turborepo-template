...
例として、`react-ui` パッケージの `package.json` ファイルは次のように設定されています：

<Callout type="info">
  この設定により、`pnpm` で `react-ui` パッケージを依存関係に持つ他のパッケージやアプリケーションが
  `react-ui` をインストールする際、ビルド済み成果物（`dist`）が利用されます。
  つまり、`react-ui` を利用する他のパッケージやアプリケーションの `node_modules/@repo/react-ui` ディレクトリには
