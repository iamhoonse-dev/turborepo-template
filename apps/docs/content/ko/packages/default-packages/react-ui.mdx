...
`react-ui` 패키지는 React 컴포넌트 라이브러리로, UI 개발을 단순화하고 재사용 가능한 컴포넌트를 제공해요.

## 🎯 주요 목표

React 컴포넌트 형태로 사용 가능한 UI 패키지를 새로 추가하고 싶을 때 이 패키지를 복사하여 개발의 시작점으로 삼을 수 있도록,
아래와 같은 목적들을 가지고 작성되었어요:

- Tailwind CSS 기반의 스타일링을 지원하도록 설정
- ShadCN UI 컴포넌트들을 베이스 컴포넌트로 활용할 수 있도록 설정
- Hooks, Context API, 유틸리티 타입 등 React 에서 제공되는 API 들을 활용할 수 있도록 React, ReactDOM 을 외부 의존성으로 설정
- JSX 변환을 지원하기 위한 TypeScript 설정
...
각 기능별로 별도의 엔트리(`shadcn-ui`, `utils`, `components`, `constants`, `base.css`)를 제공하며, 빌드 산출물은 `dist` 폴더에 ES 모듈(`.es.js`)과 CommonJS(`.cjs.js`) 형식으로 생성됩니다.

### package.json

- **exports**: 각 기능별 엔트리(`shadcn-ui`, `utils`, `components`, `constants`, `base.css`)를 분리해 타입 선언(`.d.ts`), ES 모듈, CJS 모듈을 모두 제공합니다.
- **publishConfig**: `"access": "public"`으로 npm 공개 배포가 가능하도록 설정되어 있습니다.
- **files**: 빌드 산출물(`dist`)만 패키지에 포함되도록 지정합니다.
- **peerDependencies**: `react`, `react-dom`을 외부 의존성으로 명시하여, 프로젝트에서 직접 설치하도록 요구합니다.
- **scripts**: Vite와 TypeScript를 활용한 빌드, 타입 체크, 테스트 등 다양한 개발 스크립트가 정의되어 있습니다.

### vite.config.ts

- **멀티 엔트리 빌드**: `runWithGlob` 유틸을 활용해 `src` 폴더 내의 각 엔트리 파일을 자동으로 감지하여 빌드 엔트리로 등록합니다.
...
- **외부 의존성**: `react`, `react-dom`, `react/jsx-runtime`은 번들에 포함하지 않고 외부 의존성으로 처리합니다.

### tsconfig.json

- **모듈 해석**: `"moduleResolution": "bundler"`로 번들러 친화적인 모듈 해석을 사용합니다.
- **타입 지원**: 최신 ECMAScript, DOM, React 관련 타입을 지원하도록 설정되어 있습니다.
- **JSX**: `"jsx": "react-jsx"`로 React 17+ JSX 변환을 지원합니다.
