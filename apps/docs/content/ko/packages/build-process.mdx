...
예시로, `react-ui` 패키지의 `package.json` 파일을 살펴보면 다음과 같이 설정되어 있습니다:

<Callout type="info">
  이러한 설정에 의해 `pnpm`은 `react-ui` 패키지를 의존성으로 갖는 다른 패키지나 어플리케이션에서
  `react-ui` 패키지를 설치할 때, 빌드된 결과물(`dist`)을 사용하게 됩니다.
  즉, `react-ui` 패키지를 사용하는 다른 패키지나 어플리케이션의 `node_modules/@repo/react-ui` 디렉토리에는
